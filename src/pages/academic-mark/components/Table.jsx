...
function Table() {
  ...
  useEffect(() => {
    students?.forEach((element, index) => {
      ...
      formik.setFieldValue(
        `students.${index}.final_term_points`,
        element["final_term_points"],
        false
      );
      formik.setFieldValue(
        `students.${index}.first_term_points`,
        element["first_term_points"],
        false
      );
      formik.setFieldValue(
        `students.${index}.second_term_points`,
        element["second_term_points"],
        false
      );
    });
    setChanged(false);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setLoader(true);
      await onSubmit(values.students);
      setLoader(false);
      setLoading(true);
      await loadItems(
        new URLSearchParams({
    ...
    <form onSubmit={formik.handleSubmit} className="my-4 overflow-x-auto relative">
      ...
      <div className="fixed right-4 my-2 h-[50px] flex items-center justify-end">
        <button
          ...
          type="submit"
          className={`${!academic_changed && "opacity-60 pointer-events-none"
            } sticky right-0  text-white bg-gradient-to-r from-green-500 via-green-600 to-green-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg sm:px-5 sm:py-2.5 text-center xs:px-3 xs:py-2 xs:text-xs`}
        >
          {isLoading ? (
            <>
              <svg
